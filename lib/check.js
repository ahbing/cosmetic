  ...
  getRemoteData(url, function(json) {
    ...
    productProxy.updateProductStocks(product.productId, newData).then(function(newProduct) {
      ...
      for(let i = 0, len = oldData.length; i < len; i++) {
        ...
        if (!oldItemData.inStock || oldItemData.qty === 0) {
          // 如果原来库存为零
          let colourValue = oldItemData.value;
          if (colours.some(function(e) { return e === colourValue })) {
            outOfStock.push({
              index: i,
      ...
      if(outOfStock.length) {
        outOfStock.forEach(function(item) {
          ...
          if (newItemData.qty >= threshold && newItemData.inStock && !oldItemData.inStock) {
            // 组装通知内容
            ...
            let fields = {
              ...
              to: mailTo,
              html: `
              <p>你关注的商品${productName}的${newItemData.value}款，当前库存量为<b>${newItemData.qty}</b>，点击下面链接购买</p>
              <a href="${link}">购买链接</a>
