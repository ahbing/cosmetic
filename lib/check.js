      ...
      if(outOfStock.length) {
        outOfStock.forEach(function(item) {
          ...
          if (newItemData.sku === oldItemData.sku && newItemData.qty >= threshold) {
            // 组装通知内容
            let link = `http://www.selfridges.com/CN/en/cat/${title}-${brand}_${product.wcid}/?previewAttribute=${value}`;
            ...
            // 添加验证
            let addBagUrl = `http://www.selfridges.com/HK/en/webapp/wcs/stores/servlet/BasketAdd?&productId=${product.productId}&storeId=${product.storeId}&quantity=1&Colour=${value}`;
            let options = {
              url: addBagUrl,
              headers: {
                'User-Agent': 'request'
