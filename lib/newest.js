    ...
    for (let i = 0; i < oldLen; i++) {
      ...
      if (oldItem['token'] !== newItemToken) {
        ...
        Object.assign(fields, {
          ...
          <p>检测到的时间为：${new Date()} </p>
          <a target="_blank" href="${url}">直达上新商品链接：${url}</a>
          <br/>
          <br/>
          <a target="_blank" href="${link}">上新页面链接：${link}</a>
          <p>点此链接访问管理界面<a target="_blank" href=${config.domain}>${config.domain}</a></p>
          `
        });
