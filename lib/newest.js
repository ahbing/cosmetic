...
let reqCallback = function(err, response, body) {
  ...
  if (!oldLen) {
    oldProducts = newLen ? newProducts : [];
  } else {
    for (let i = 0; i < oldLen; i++) {
      ...
      for (let j = 0; j < newLen; j++) {
        ...
        if (oldItem['index'] == newItem['index']) {
          // console.log(oldItem['title'],'=======', newItem['title'])
          // console.log(oldItem['des'],'=======', newItem['des'])
          if (oldItem['title'] !== newItem['title'] || oldItem['des'] !== newItem['des']) {
            Object.assign(fields, {
              subject: `上新了${newItem['title']}`,
              to: mailTo,
              html: `
              <p>检测到${newItem['title']}：${newItem['des']}上架了，点击下面链接购买</p>
              <a href="${link}">购买链接</a>
            ...
            send(fields);
            // 提前结束
            return olProducts = newProducts;
          }
        }
      }
    }
    olProducts = newProducts;
  }
}
